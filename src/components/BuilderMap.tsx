...
export interface BuilderPin {
  ...
  /**
   * Pre-computed profile URL for this pin. Parent calls
   * getShopProfileUrl(shop, basePath) when building the pins array.
   */
  profile_href?: string;
}
...
export default function BuilderMap({
  ...
  useEffect(() => {
    ...
    function buildPopup(b: BuilderPin & { latitude: number; longitude: number }): string {
      ...
      const href =
        b.profile_href ?? `/builders/${stateToSlug(b.state)}/${b.slug}/`;
      const stars =
      ...
      return (
        ...
        stars +
        `<a href="${href}" style="display:inline-block;margin-top:6px;font-size:12px;font-weight:500;color:#14331E;text-decoration:none;">View Profile →</a>` +
        `</div>`
      );
    }
